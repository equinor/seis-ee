...
good_payload = """abc
test/file.txt
_sdfg_9971-12/kjlskgdjk2/akhfd.3128sadnjk
s
gh
.tjs
/gs/asd/.sds
...
bad_payload = """&lt;!--#exec%20cmd=&quot;/bin/cat%20/etc/passwd&quot;--&gt;
...
class TestCommandInjectionSanitizing(unittest.TestCase):
    def test_many_valid_string(self):
        accepted = []
        for line in good_payload.splitlines():
            accepted.append(sanitize_shell_arguments(line))
        assert len(accepted) == len(good_payload.splitlines())

    def test_many_invalid_strings(self):
        accepted = []
        for line in bad_payload.splitlines():
            with raises(BadInputException):
                accepted.append(sanitize_shell_arguments(line))
        assert not accepted
